          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {loading ? (
              ...
              <div key={m.id} className="group bg-white rounded-2xl border border-slate-200 p-5 hover:shadow-xl transition-all duration-300 relative overflow-hidden">
                <div className={`absolute top-0 right-0 h-1.5 w-full ${m.status === 'approved' ? 'bg-green-500' : 'bg-amber-500'}`} />
                ...
                <div className="flex flex-col items-center">
                  <div className="relative mt-2">
                    ...
                    <span className={`absolute bottom-0 right-0 w-5 h-5 rounded-full border-2 border-white ${m.status === 'approved' ? 'bg-green-500' : 'bg-amber-500'}`} />
                  </div>
                ...
                <div className="flex border-b border-slate-100 px-6 overflow-x-auto no-scrollbar bg-white">
                  {['overview', 'garage', 'skills', 'documents', 'payout'].map((tab) => (
                    <button
                      ...
                      className={`py-4 px-4 text-sm font-bold border-b-2 transition-all capitalize whitespace-nowrap ${
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div className="space-y-6">
                        ...
                        <div className="grid gap-4">
                          ...
                          <InfoCard label="Experience Level" value={selectedMechanic.skills?.experience} icon={<Briefcase size={16}/>} />
                          <InfoCard label="User ID" value={selectedMechanic.id} icon={<FileText size={16}/>} />
                        </div>
                      </div>
                      <div className="space-y-6">
                        <SectionTitle icon={<ShieldCheck size={18}/>} title="Verification Logs" />
                        <div className="p-5 bg-slate-50 rounded-2xl border border-slate-100 border-l-4 border-l-blue-500">
                          ...
                          <p className="text-slate-700 italic text-sm">"{selectedMechanic.adminRemark || 'No remarks provided yet.'}"</p>
                    ...
                    <div className="space-y-8">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="space-y-4">
                          <InfoCard label="Aadhaar Number" value={selectedMechanic.kyc?.aadhaarNumber} />
                          <div className="flex gap-2">
                             <DocBadge label="PAN" exists={!!selectedMechanic.kyc?.panImage} />
                             <DocBadge label="LICENSE" exists={!!selectedMechanic.kyc?.licenseImage} />
                          </div>
                        </div>
                        {selectedMechanic.kyc?.aadhaarImage && (
                          <div className="space-y-2">
                            <p className="text-[10px] font-bold text-slate-400 uppercase">Aadhaar Card Preview</p>
                            <a href={selectedMechanic.kyc.aadhaarImage} target="_blank" rel="noreferrer">
                               <img src={selectedMechanic.kyc.aadhaarImage} className="w-full max-w-sm rounded-xl border-4 border-slate-50 shadow-lg hover:brightness-90 transition-all cursor-zoom-in" />
                            </a>
                          </div>
                        )}
                      </div>
